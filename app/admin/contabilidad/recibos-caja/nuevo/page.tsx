...
// Crear Recibo de Caja manual (sin cotización opcional, sin pago previo).
// El email NO se envía automáticamente — se dispara desde el detalle.
// ============================================================================
...
export default function NuevoReciboCajaPage() {
  ...
  // Resetear cotización si cambia cliente
  useEffect(() => {
    setCotizacionId('');
  }, [clienteId]);
  ...
  const guardar = useCallback(async () => {
    ...
    await mutate('/api/admin/contabilidad/recibos-caja', {
      ...
      onSuccess: (data: any) => {
        router.push(`/admin/contabilidad/recibos-caja/${data.id}?creado=1`);
      },
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-5">
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-slate-400">Aparece tal cual en el PDF y en el email del recibo.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {montoNum > 0 && (
              <p className="mt-1 text-xs text-slate-500">{Q(montoNum)}</p>
            )}
          </div>
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        <div className="flex items-center justify-between gap-4">
          <p className="text-xs text-slate-500">
            El email <strong>NO</strong> se envía al guardar. Una vez creado, podrás enviarlo desde el detalle del recibo.
          </p>
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-[#059669] to-[#10B981] rounded-lg hover:shadow-lg hover:shadow-emerald-900/20 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            >

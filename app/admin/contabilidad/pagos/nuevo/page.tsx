...
export default function NuevoPagoPage() {
  ...
  // Can submit?
  const canSubmit = modo === 'cotizacion'
    ? montoNum > 0 && !!clienteId && !guardando
    : montoNum > 0 && !!concepto.trim() && !guardando;
  ...
  const guardar = useCallback(async () => {
    if (modo === 'cotizacion' && !clienteId) return alert('Selecciona una cotización');
    ...
    // Build notas from concepto + pagadoPor + notas
    let notasFinal = '';
    if (modo === 'directo') {
      const parts: string[] = [];
      if (concepto.trim()) parts.push(concepto.trim());
      if (pagadoPor.trim()) parts.push(`Pagado por: ${pagadoPor.trim()}`);
      if (notas.trim()) parts.push(notas.trim());
      notasFinal = parts.join('\n');
    } else {
      notasFinal = notas || '';
    }
  ...
  }, [clienteId, montoNum, metodoPago, referencia, esAnticipo, notas, confirmarInmediato, modo, cotizacionSeleccionada, concepto, pagadoPor, mutate, router]);
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        {modo === 'directo' && (
          <div className="space-y-4">
            ...
            {/* Pagado por (optional — for third-party payers) */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
                Pagado por
                <span className="text-xs text-slate-400 font-normal ml-1">(opcional — si paga un tercero)</span>
              </label>
              <input
                type="text"
                value={pagadoPor}
                onChange={e => setPagadoPor(e.target.value)}
                placeholder="Ej: Juan Pérez (inquilino), María López..."
                className="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0891B2]/20 focus:border-[#0891B2]"
              />
            </div>
          </div>
          ...
          <div>
            {montoNum > 0 && (
              <div className="text-sm text-slate-600">
                ...
                {modo === 'directo' && clienteNombre && (
                  <>
                    <span className="mx-2">→</span>
                    <span>{clienteNombre}</span>
                  </>
                )}
                {modo === 'directo' && !clienteNombre && pagadoPor.trim() && (
                  <>
                    <span className="mx-2">→</span>
                    <span>{pagadoPor.trim()}</span>
                  </>
                )}
